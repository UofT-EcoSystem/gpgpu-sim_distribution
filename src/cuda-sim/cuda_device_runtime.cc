...
void gpgpusim_cuda_getParameterBufferV2(const ptx_instruction *pI,
    ...
    unsigned child_kernel_arg_size =
        child_kernel_entry->get_args_aligned_size();
    ...
    for (unsigned arg = 0; arg < n_args; arg++) {
        ...
        if (arg == 0) { // paramter buffer for child kernel (in global memory)
            ...
            if (device_kernel_entry->is_pdom_set()) {
                ...
                printf("GPGPU-Sim PTX: finding reconvergence points for "
                       "\'%s\'...\n",
                       device_kernel_entry->get_name().c_str());
                /*
                 * Some of the instructions like printf() gives the gpgpusim the
                 * wrong impression that it is a function call. As printf()
                 * doesnt have a body like functions do, doing pdom analysis for
            ...
            DEV_RUNTIME_REPORT("device_kernel_arg_size "
                               << device_kernel_arg_size);
            memory_space *device_kernel_param_mem;

            // create child kernel_info_t and index it with parameter_buffer
            // address
            ...
            DEV_RUNTIME_REPORT(
                ...
                << ", " << thread->get_ctaid().y << ", "
                << thread->get_ctaid().z << "), thread (" << thread->get_tid().x
            ...
            device_kernel_param_mem =
                device_grid->get_param_memory(); // kernel param
            ...
            for (unsigned n = 0; n < device_kernel_arg_size; n += 4) {
                ...
                device_kernel_param_mem->write(param_start_address + n, 4,
                                               &oneword, NULL, NULL);
            }
...
void gpgpusim_cuda_streamCreateWithFlags(const ptx_instruction *pI,
    ...
    for (unsigned arg = 0; arg < n_args; arg++) {
        ...
        if (arg == 0) { // cudaStream_t * pStream, address of cudaStream_t
            ...
            pStream_addr =
                generic_to_local(thread->get_hw_sid(), thread->get_hw_tid(),
                                 generic_pStream_addr);

            DEV_RUNTIME_REPORT("pStream locating at local memory "
                               << pStream_addr);
        } else if (arg ==
    ...
    thread->m_local_mem->write(pStream_addr, sizeof(cudaStream_t), &stream,
                               NULL, NULL);
...
void launch_one_device_kernel() {
    if (!g_cuda_device_launch_op.empty()) {
        ...
        stream_operation stream_op =
            stream_operation(op.grid, g_ptx_sim_mode, op.stream);
